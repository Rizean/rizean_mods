...
// path D:\other\LifePlay_4_15_64bit\LifePlay\Content\Modules\vin_Base\Scenes\hourly\bartender_fucks.lpscene

const {LPScene} = require('lifeplayjs')

const scene = new LPScene({name: 'bartender_fucks'}, (scene) => {
    ...
    scene.start(() => {
        ...
        narrative(`The couple I was serving at the bar was arguing the whole time. The alcohol clearly made it worse. They are causing quite a SCENE.`)
        Actor.hide()
        ...
        if (0) {
            ...
            if (0) {
                ...
                narrative(`We chat for a while more before exchanging numbers. Seeing now that <Actor.name> will inevitably break up with <Actor.his_or_her> <Actor.boyfriend_or_girlfriend>, maybe there could be something between us.`)
                ...
                if (random(0, 100) < Actor.perversion + Actor.attractionToPlayer) {
                    ...
                    Actor.show(2)
                    narrative(`Will you exchange number with <Actor.name>?`)
                    option([
                        {text: `Yes`},
                        {text: `No`},
                    ])
                    if (0) {
                        ...
                        narrative(`I must have make some impression to <Actor.name>. We exchanged numbers so we can hang at later date.`)
                        Player.exchangeContact(Actor)
                    }
                } else {
                    Actor.dialogue(`Yet I thought for a moment there that you were a nice <Actor.guy_or_girl>. I'm pissed at my <Actor2.boyfriend_or_girlfriend>, but that doesn't mean you can take advantage of me!`, `Angry`)
                }
            }
        } else {
    ...
    scene.timeout(200, "bartender_fucks")
})
