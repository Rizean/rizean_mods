...
// path D:\other\LifePlay_4_15_64bit\LifePlay\Content\Modules\vin_Corruption\Scenes\co_ag_erotica.lpscene

const {LPScene} = require('lifeplayjs')

const scene = new LPScene({name: 'co_ag_erotica'}, (scene) => {
    ...
    scene.start(() => {
        ...
        narrative(`Maybe a perverted gift like an erotic book would encourage <Actor.him_or_her> to open up <Actor.his_or_her> mind a bit ...`)
        option([
            {text: `Send <Actor.name> this gift`},
            {text: `That's a bad idea`},
        ])
        if (0) {
            Player.money -= random(10, 40)
    ...
    scene.timeout(200, "co_ag_erotica")
})
