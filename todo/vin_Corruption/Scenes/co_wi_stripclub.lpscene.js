...
// path D:\other\LifePlay_4_15_64bit\LifePlay\Content\Modules\vin_Corruption\Scenes\co_wi_stripclub.lpscene

const {LPScene} = require('lifeplayjs')

const scene = new LPScene({name: 'co_wi_stripclub'}, (scene) => {
    const {Player, none, $WHERE, narrative, option, random, WHEN} = scene

    scene.what(["all"])
    scene.where(["all", " -home", " -stripclub"])
    scene.when([20, 3])
    ...
    scene.start(() => {
        ...
        narrative(`They're heading into a small street - I know this one. There's a well-known stripclub in there!`)
        option([
    ...
    scene.timeout(1000, "co_wi_stripclub")
})
