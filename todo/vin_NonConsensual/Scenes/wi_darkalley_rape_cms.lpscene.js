...
// path D:\other\LifePlay_4_15_64bit\LifePlay\Content\Modules\vin_NonConsensual\Scenes\wi_darkalley_rape_cms.lpscene

const {LPScene} = require('lifeplayjs')

const scene = new LPScene({name: 'wi_darkalley_rape_cms'}, (scene) => {
    ...
    scene.start(() => {
        let Actor = scene.generatePersonTemporary(["wi_darkalley_rape_cms"])
        let Known = false
        ...
        Actor = scene.generatePersonTemporary(["wi_darkalley_rape_cms"])
        Actor.randomizeFace()
        Actor.randomizeHairs()
        Actor.dressUniform("Crime")
        Actor.show(2)
        Known = false
        if (scene.isModEnabled("vin_NTR") && random(0, 100) < 50) {
            if (random(0, 100) < 40) {
                scene.$random(() => {
        ...
        if (Known) {
            narrative(`Oh no, <Actor2.name> is getting mugged and threatened with rape ...`)
        } else {
            narrative(`Oh no, that poor <Actor2.guy_or_girl> is getting mugged and threatened with rape ...`)
        }
        narrative(`What should do?`)
        option([
            {text: `Call the cops`},
            {text: `Run`},
            {text: `Attack`},
    ...
    scene.timeout(500, "wi_darkalley_rape_cms")
})
