...
// path D:\other\LifePlay_4_15_64bit\LifePlay\Content\Modules\vin_NonConsensual\Scenes\wi_bathroom_rape.lpscene

const {LPScene} = require('lifeplayjs')

const scene = new LPScene({name: 'wi_bathroom_rape'}, (scene) => {
    ...
    scene.start(() => {
        ...
        narrative(`What's that sound coming from the <Actor2.male_or_female> bathroom? Is that ... a scream? I need to check it out, sounds like someone's in serious distress.`)
        Actor = scene.generatePersonTemporary([])
        Actor2.show(3)
        Actor.show(2)
        narrative(`Oh my god, someone is being sexually assaulted in one of the cubicles! I must do something!`)
        option([
        ...
        if (0) {
            ...
            narrative(`I decided to run and call for help from the place's staff. Eventually, they overwhelmed the rapist and rescued the poor <Actor2.guy_or_girl>. It was sorta too late though, as by the time help arrives, the rapist had already came inside <Actor.his_or_her> poor victim.`)
        } else if (1) {
            ...
            if (Player.martial > Actor.martial || !scene.isModEnabled("vin_NTR")) {
                ...
                if (0) {
                    Player.dialogue(`It's only my citizen's duty. I cannot let criminals like that get away with their creepy acts.`, `Happy`)
                } else {
        ...
        if (Rape) {
            ...
            narrative(`<Actor2.His_or_Her> cries for help went unheard in the bathroom and <Actor2.he_or_she> had no choice but endure it all at the hand of this vile <Actor.man_or_woman> in a dirty cubicle.`)
            scene.filter("Aggressive")
            scene.talkNonConsensual()
            scene.sexNoAffair(Actor, Actor2)
        }
    })
    scene.timeout(500, "wi_bathroom_rape")
})
