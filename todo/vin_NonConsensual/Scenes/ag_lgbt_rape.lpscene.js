...
// path D:\other\LifePlay_4_15_64bit\LifePlay\Content\Modules\vin_NonConsensual\Scenes\ag_lgbt_rape.lpscene

const {LPScene} = require('lifeplayjs')

const scene = new LPScene({name: 'ag_lgbt_rape'}, (scene) => {
    ...
    scene.who(($IF) => {
        ...
        $IF(!Actor.isRelative() && !Actor.isInterestedIn(Player))
    })
    scene.other(($IF) => {
        $IF(Player.isInterestedIn("Actor") && random(70, 300) < Actor.attractiveness)
    })


    scene.start(() => {
        narrative(`<Actor.name> is smoking hot ... What a shame that <Actor.he_or_she> isn't into <Player.guys_or_girls> ...`)
        option([
            {text: `<Actor.He_or_She> was born that way`},
            {text: `I bet I can 'convert' <Actor.him_or_her>`},
        ])
        if (0) {
            narrative(`That's just <Actor.his_or_her> sexual orientation. I have to respect that. There are a million other <Actor.guys_or_girls> out there for me.`)
        } else {
            narrative(`<Actor.He_or_She> might not be into <Player.guys_or_girls> ... for now. I bet I can convert <Actor.him_or_her>.`)
            ...
            if (0) {
                narrative(`Some good old alcohol should do the trick in bringing <Actor.him_or_her> to <Actor.his_or_her> true sexuality.`)
                ...
                if (0) {
                    ...
                    Actor.dialogue(`Eww <Player.name>! What the hell are you doing? You know I'm not into <Player.guys_or_girls>!`, `Angry`)
                    ...
                    if (0) {
                        ...
                        Player.karma -= 20
                        Player.dialogue(`Nobody rejects me like that! Let's see if I can't convert you after a hard pounding!`, `Furious`)
                        ...
                        if (Actor.martial * 0.5 > Player.martial) {
                            ...
                            Actor.dialogue(`No! Please stop ... Somebody help ...`, `Crying`)
                        ...
                        if (random(0, 100) < 85) {
                            narrative(`I was lucky to avoid jail time as <Actor.name> never reported me to the police, but <Actor.he_or_she> blocked me and never wanted to see me again. One thing for sure: I did nothing to convert <Actor.name> to become interested in <Player.guys_or_girls>.`)
                        } else {
    ...
    scene.timeout(500, "ag_lgbt_rape")
})
