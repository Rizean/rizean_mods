...
// path D:\other\LifePlay_4_15_64bit\LifePlay\Content\Modules\vin_Base\Scenes\dating\breakup_cheating.lpscene

const {LPScene} = require('lifeplayjs')

const scene = new LPScene({name: 'breakup_cheating'}, (scene) => {
    const {Player, none, $WHERE, narrative, option, random, WHEN, actionDuration, choice} = scene

    scene.what(["all", " -sleep"])
    scene.where(["home"])
    scene.when([0, 24])
    ...
    scene.who(($IF) => {
        ...
        $IF(Dating.intelligence + Dating.interpersonal - Dating.masochist > random(0, 1000) * actionDuration)
    })
    scene.other("none")
    ...
    scene.start(() => {
        ...
        Dating.dialogue(`<Player.name>, we need to talk!`, `Angry`)
        ...
        Dating.dialogue(`Of course I don't, and you know damn well why!`, `Angry`)
        Dating.dialogue(`I know about you and <Affair.name>. You <Player.asshole_or_slut>, you were sleeping around behind my back!`, `Furious`)
        option([
            {text: `Deny`},
        ...
        if (0) {
            Player.karma -= 2.5
            Player.dialogue(`Baby! You must trust me. Don't believe those baseless rumours. <Affair.name> and I - we didn't do anything!`, `Angry`)
            if (Player.intelligence - Dating.intelligence > random(0, 50)) {
                Dating.dialogue(`Maybe you're right. I'm sorry - I shouldn't have believed those lies ...`, `Sad`)
                breakup = false
            } else {
        ...
        if (breakup) {
            if (Dating.masochist + Player.karma < -50) {
                Dating.animate("gun")
                Dating.moveToPerson(Player)
                Dating.dialogue(`You betrayed me, you humilated me ... now you die!`, `Furious`)
            ...
            if (0) {
                ...
                Dating.dialogue(`More important than your <Dating.boyfriend_or_girlfriend> apparently ... Fine, then it's over between us!`, `Furious`)
                if (Player.isPlayerMarried()) {
                    Dating.dialogue(`I'm afraid this marriage isn't working ...`, `Sad`)
                    narrative(`And just like that, <Dating.name> divorced me ...`)
                    Player.mood -= 100
