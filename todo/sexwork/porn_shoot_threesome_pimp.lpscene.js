...
// path D:\other\LifePlay_4_15_64bit\LifePlay\Content\Modules\vin_Base\Scenes\sexwork\porn_shoot_threesome_pimp.lpscene

const {LPScene} = require('lifeplayjs')

const scene = new LPScene({name: 'porn_shoot_threesome_pimp'}, (scene) => {
    ...
    scene.who(($IF) => {
        ...
        $IF(random(1, 300) < Actor3.pornfame)
    })
    scene.other("none")


    scene.start(() => {
        let Dice = random(750, 3000)
        ...
        Actor2.dress()
        Actor2.show(3)
        narrative(`A porn director wants to book <Actor3.name> for a threesome SCENE with <Actor.name> and <Actor2.name>...`)
        option([
        ...
        if (0) {
            ...
            Actor3.pornfame += 0.6
            Actor3.perversion += 0.35
        } else {
            narrative(`No way, that's way above the market rate for this type of shoots! You have too high an opinion of talent. You should ask the other agents how much they charge.`)
    ...
    scene.timeout(48, "porn_shoot_threesome_pimp")
})
