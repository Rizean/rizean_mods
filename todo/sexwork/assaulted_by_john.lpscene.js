...
// path D:\other\LifePlay_4_15_64bit\LifePlay\Content\Modules\vin_Base\Scenes\sexwork\assaulted_by_john.lpscene

const {LPScene} = require('lifeplayjs')

const scene = new LPScene({name: 'assaulted_by_john'}, (scene) => {
    ...
    scene.start(() => {
        ...
        narrative(`A John is requesting an outcall. <Actor.He_or_She> wants to meet me in a motel room in quite a remote location ... <Actor.He_or_She> promised to be very generous.`)
        option([
            {text: `Sure, probably just a discreet married customer`},
            {text: `No way, sounds too dodgy`},
        ])
        if (0) {
            scene.setBackground("home")
            Player.dialogue(`So here's the room ... it's rather disgusting actually ... I hope the customer pays well enough to make up for it.`)
            Actor.show()
            Actor.dialogue(`Surprise!`)
            narrative(`Suddenly, the customer appeared out of nowhere, butt-naked.`)
    ...
    scene.timeout(300, "assaulted_by_john")
})
