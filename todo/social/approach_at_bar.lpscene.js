...
// path D:\other\LifePlay_4_15_64bit\LifePlay\Content\Modules\vin_Base\Scenes\social\approach_at_bar.lpscene

const {LPScene} = require('lifeplayjs')

const scene = new LPScene({name: 'approach_at_bar'}, (scene) => {
    const {Player, none, $WHERE, narrative, option, random, WHEN, choice} = scene

    scene.what(["all", " -go_to_the_bathroom", " -dance", " -have_a_meal", " -have_a_snack"])
    scene.where(["nightclub", " bar", " pub"])
    scene.when([0, 24])
    scene.who("none")
    scene.other(($IF) => {
        $IF(!Player.isWithCompanion() && random(0, 500) < Player.attractiveness + Player.interpersonal)
    })
    ...
    scene.start(() => {
        ...
        if (Actor.isDominantSex(Player) && random(0, 70) < Actor.interpersonal) {
            Actor.dialogue(`Hey there, how is it going?`, `Happy`)
            narrative(`<Actor.name> approached me and tried to start a conversation. Should I stay and chat?`)
        } else {
